...
DomError1(N,j,Net)$(par(net) eq 1)..TempL(N,j,Net)/Tcritm(N,j,Net)=l=1-(1e-3);


positive variable rhoV(N,j,Net) "Densidad molar de vapor [mol/m^3]";
equation EqurhoV(N,j,Net);
EqurhoV(N,j,Net)$(par(net) eq 1).. rhoV(N,j,Net)=e=P(N,j,Net)/(0.00008314*TempV(N,j,Net)*(Z(N,j,Net)));
...
Eq_tao_nrt2I(N,j,comp,comp1,Net)$(ord(comp) eq ord(comp1) and par(net) eq 1).. tao_nrtlI(N,j,comp,comp1,Net)=e=0;

variables g_nrtlI(N,j,comp,comp1,Net);
equations Eq_g_nrtlI(N,j,comp,comp1,Net),Eq_g_nrt2I(N,j,comp,comp1,Net);
Eq_g_nrtlI(N,j,comp,comp1,Net)$(ord(comp) ne ord(comp1) and par(net) eq 1).. g_nrtlI(N,j,comp,comp1,Net)=e=exp( -alfa_nrtl(comp,comp1)*tao_nrtlI(N,j,comp,comp1,Net));
Eq_g_nrt2I(N,j,comp,comp1,Net)$(ord(comp) eq ord(comp1) and par(net) eq 1).. g_nrtlI(N,j,comp,comp1,Net)=e=1;

*Coeficiente de actividad equilibrio(gammaI)
variables gammaI(N,j,comp,Net);
equations EqgammaI(N,j,comp,Net);
EqgammaI(N,j,comp,Net)$(par(net) eq 1).. gammaI(N,j,comp,Net)=e=
...
EqAlphaEOS(N,j,comp,Net)$(par(net) eq 1).. alphaEOS(N,j,comp,Net) =e= sqr(1+mEOS(comp)*(1-sqrt(TempV(N,j,Net)/Tcritm(N,j,Net))));
...
EqaEOS(N,j,Net)$(par(net) eq 1).. aEOS(N,j,Net) =e= sum(comp,sum(comp1, (y(N,j,comp,Net)/100)*(y(N,j,comp1,Net)/100)*sqrt(aiEOS(N,j,comp,Net)*aiEOS(N,j,comp1,Net))));


equation VaporZ(N,j,Net);
VaporZ(N,j,Net)$(par(net) eq 1).. power(Z(N,j,Net),3)-sqr(Z(N,j,Net))+(Z(N,j,Net))
...
EqPhi(N,j,comp,Net)$(par(net) eq 1).. phiI(N,j,comp,Net) =e= exp(((Z(N,j,Net))-1)*biEOS(comp)/bEOS(N,j,Net)
...
VaporZb(N,j,comp,Net)$(par(net) eq 1).. power(Zboil(N,j,comp,Net),3)-sqr(Zboil(N,j,comp,Net))+(Zboil(N,j,comp,Net))
...
VaporZbut(N,j,Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1) and yf2(net) eq 1).. power(Zbut(N,j,Net),3)-sqr(Zbut(N,j,Net))+(Zbut(N,j,Net))
...
defmatRvap(N,j,net,compR,compR1,compREF)$( (ord(Net)>1 and ord(Net)<card(Net)) and (ord(compR) ne ord(compR1)) and par(net) eq 1)..0=e=100*matRvap(N,j,net,compR,compR1)+y(N,j,compR,net)*((1/kbinVap(N,j,net,compR,compR1))-(1/kbinVap(N,j,net,compR,compREF)));
defmatRvapDiag(N,j,net,compR,compR1,compREF)$( (ord(Net)>1 and ord(Net)<card(Net)) and (ord(compR) eq ord(compR1)) and par(net) eq 1)..0=e=100*matRvap(N,j,net,compR,compR1)-(((y(N,j,compR,net))/(kbinVap(N,j,net,compR,compREF)))+sum(comp$(ident(comp) ne ident(compR)),(y(N,j,comp,net))/(kbinVap(N,j,net,compR,comp))));

equation defmatRliq(N,j,net,compR,compR1,compREF),defmatRliqDiag(N,j,net,compR,compR1,compREF);
defmatRliq(N,j,net,compR,compR1,compREF)$( (ord(Net)>1 and ord(Net)<card(Net)) and (ord(compR) ne ord(compR1)) and par(net) eq 1)..0=e=100*matRliq(N,j,net,compR,compR1)+x(N,j,compR,net)*((1/kbinliq(N,j,net,compR,compR1))-(1/kbinliq(N,j,net,compR,compREF)));
defmatRliqDiag(N,j,net,compR,compR1,compREF)$( (ord(Net)>1 and ord(Net)<card(Net)) and (ord(compR) eq ord(compR1)) and par(net) eq 1)..0=e=100*matRliq(N,j,net,compR,compR1)-(((x(N,j,compR,net))/(kbinliq(N,j,net,compR,compREF)))+sum(comp$(ident(comp) ne ident(compR)),(x(N,j,comp,net))/(kbinliq(N,j,net,compR,comp))));
...
defktransVap(N,j,net,compR,compR1)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..0=e=sum(compR2,ktransVap(N,j,net,compR,compR2)*matRvap(N,j,net,compR2,compR1))-termofacVap(N,j,net,compR,compR1);
defktransLiq(N,j,net,compR,compR1)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..0=e=sum(compR2,ktransLiq(N,j,net,compR,compR2)*matRliq(N,j,net,compR2,compR1))-termofacliq(N,j,net,compR,compR1);
...
defmuL(N,j,comp,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..muL(N,j,comp,net)=e=exp(C1MLiq(comp)+(C2MLiq(comp)/TempL(N,j,net))+C3MLiq(comp)*log(TempL(N,j,net)));
defmuV(N,j,comp,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..muV(N,j,comp,net)=e=(C1MVap(comp)*(TempV(N,j,net)**C2MVap(comp)))/(1+(C3MVap(comp)/TempV(N,j,net)));
...
defmumixL(N,j,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..mumixL(N,j,net)=e=exp(sum(comp,(x(N,j,comp,net)/100)*(log(muL(N,j,comp,net)))));
...
EqTcritmDew(N,j,Net)$(par(net) eq 1).. Tcritm_Dew(N,j,Net) =e= (sqr(sum(comp,(x_Dew(N,j,comp,Net)/100)*Tcrit(comp)/sqrt(Pcrit(comp)))))/(sum(comp,(x_Dew(N,j,comp,Net)/100)*Tcrit(comp)/Pcrit(comp)));

variables tao_nrtl_Bubble(N,j,comp,comp1,Net);
equations Eq_tao_nrtlBubble(N,j,comp,comp1,Net);
Eq_tao_nrtlBubble(N,j,comp,comp1,Net)$(par(net) eq 1).. tao_nrtl_Bubble(N,j,comp,comp1,Net)=e=a_nrtl(comp,comp1) + (b_nrtl(comp,comp1)/Temp_Bubble(N,j,Net));

variables g_nrtl_Bubble(N,j,comp,comp1,Net);
equations Eq_g_nrtlBubble(N,j,comp,comp1,Net);
Eq_g_nrtlBubble(N,j,comp,comp1,Net)$(par(net) eq 1).. g_nrtl_Bubble(N,j,comp,comp1,Net)=e=exp( -alfa_nrtl(comp,comp1)*tao_nrtl_Bubble(N,j,comp,comp1,Net));

variables gamma_Bubble(N,j,comp,Net);
equations EqgammaBubble(N,j,comp,Net);
EqgammaBubble(N,j,comp,Net)$(par(net) eq 1).. gamma_Bubble(N,j,comp,Net)=e=
...
Eq_tao_nrtlDew(N,j,comp,comp1,Net)$(par(net) eq 1).. tao_nrtl_Dew(N,j,comp,comp1,Net)=e=a_nrtl(comp,comp1) + (b_nrtl(comp,comp1)/Temp_Dew(N,j,Net));

variables g_nrtl_Dew(N,j,comp,comp1,Net);
equations Eq_g_nrtlDew(N,j,comp,comp1,Net);
Eq_g_nrtlDew(N,j,comp,comp1,Net)$(par(net) eq 1).. g_nrtl_Dew(N,j,comp,comp1,Net)=e=exp( -alfa_nrtl(comp,comp1)*tao_nrtl_Dew(N,j,comp,comp1,Net));

variables gamma_Dew(N,j,comp,Net);
equations EqgammaDew(N,j,comp,Net);
EqgammaDew(N,j,comp,Net)$(par(net) eq 1).. gamma_Dew(N,j,comp,Net)=e=
...
EqAlphaEOSBubble(N,j,comp,Net)$(par(net) eq 1).. alphaEOS_Bubble(N,j,comp,Net) =e= sqr(1+mEOS(comp)*(1-sqrt(Temp_Bubble(N,j,Net)/Tcritm(N,j,Net))));


positive variable aiEOS_Bubble(N,j,comp,Net);
equation EqaiEOSBubble(N,j,comp,Net);
EqaiEOSBubble(N,j,comp,Net)$(par(net) eq 1).. aiEOS_Bubble(N,j,comp,Net) =e= alphaEOS_Bubble(N,j,comp,Net)*0.42747*(sqr(0.00008314*TcritSRK(comp)))/Pcrit(comp);


positive variable bEOS_Bubble(N,j,Net);
equation EqbEOSBubble(N,j,Net);
EqbEOSBubble(N,j,Net)$(par(net) eq 1).. bEOS_Bubble(N,j,Net) =e= sum(comp,(y_Bubble(N,j,comp,Net)/100)*biEOS(comp));


positive variable aEOS_Bubble(N,j,Net);
equation EqaEOSBubble(N,j,Net);
EqaEOSBubble(N,j,Net)$(par(net) eq 1).. aEOS_Bubble(N,j,Net) =e= sum(comp,sum(comp1, (y_Bubble(N,j,comp,Net)/100)*(y_Bubble(N,j,comp1,Net)/100)*sqrt(aiEOS_Bubble(N,j,comp,Net)*aiEOS_Bubble(N,j,comp1,Net))));


equation VaporZBubble(N,j,Net);
VaporZBubble(N,j,Net)$(par(net) eq 1).. power(Z_Bubble(N,j,Net),3)-sqr(Z_Bubble(N,j,Net))+(Z_Bubble(N,j,Net))
...
EqPhiBubble(N,j,comp,Net)$(par(net) eq 1).. phi_Bubble(N,j,comp,Net) =e= exp(((Z_Bubble(N,j,Net))-1)*biEOS(comp)/bEOS_Bubble(N,j,Net)
...
EqAlphaEOSDew(N,j,comp,Net)$(par(net) eq 1).. alphaEOS_Dew(N,j,comp,Net) =e= sqr(1+mEOS(comp)*(1-sqrt(Temp_Dew(N,j,Net)/Tcritm_Dew(N,j,Net))));


positive variable aiEOS_Dew(N,j,comp,Net);
equation EqaiEOSDew(N,j,comp,Net);
EqaiEOSDew(N,j,comp,Net)$(par(net) eq 1).. aiEOS_Dew(N,j,comp,Net) =e= alphaEOS_Dew(N,j,comp,Net)*0.42747*(sqr(0.00008314*TcritSRK(comp)))/Pcrit(comp);


positive variable aEOS_Dew(N,j,Net);
equation EqaEOSDew(N,j,Net);
EqaEOSDew(N,j,Net)$(par(net) eq 1).. aEOS_Dew(N,j,Net) =e= sum(comp,sum(comp1, (y(N,j,comp,Net)/100)*(y(N,j,comp1,Net)/100)*sqrt(aiEOS_Dew(N,j,comp,Net)*aiEOS_Dew(N,j,comp1,Net))));

equation VaporZDew(N,j,Net);
VaporZDew(N,j,Net)$(par(net) eq 1).. power(Z_Dew(N,j,Net),3)-sqr(Z_dEW(N,j,Net))+(Z_Dew(N,j,Net))
...
EqPhiDew(N,j,comp,Net)$(par(net) eq 1).. phi_Dew(N,j,comp,Net) =e= exp(((Z_Dew(N,j,Net))-1)*biEOS(comp)/bEOS(N,j,Net)
...
variable partial_reaction_term(net,comp);
equations eq_ETin(net),eq_BUTin(net),eq_reflux_flow(net),eq_boilup_flow(net,net1);
equations no_ETin(net),no_BUTin(net),no_reflux_flow(net),no_boilup_flow(net,net1);
...
ValMassParcialVapor0(comp,net,net1)$((ord(Net)>1 and ord(Net)<card(Net)) and (ord(Net1) eq card(Net1)))..0=e=boilup_flow(net,net1)*y('1','1',comp,Net1)+V('1','1',Net+1)*y('1','1',comp,Net+1)-V('1','1',Net)*y('1','1',comp,Net)-100*Nbonita('1','1',comp,net);
ValmassParcialLiquid0(comp,net)$(ord(Net)>1 and ord(Net)<card(Net))..0=e=ETin(net)*ze(comp)+BUTin(net)*zb('1','1',comp)+reflux_flow(net)*x('1','1',comp,'1')+L('1','1',Net-1)*x('1','1',comp,Net-1)-L('1','1',Net)*x('1','1',comp,Net)+100*Nbonita('1','1',comp,net)+100*partial_reaction_term(net,comp);
...
Notrans(comp,net)$(ord(Net)>1 and ord(Net)<card(Net) and (1-ye(net)) eq 1)..0=e=Nbonita('1','1',comp,net);
Trans1Vap(compR,net)$(ord(Net)>1 and ord(Net)<card(Net) and ye(net) eq 1)..0=e=(  100*Nbonita('1','1',compR,net)-rhoV('1','1',net)*aI('1','1',net)*sum(compR1,ktransVap('1','1',net,compR,compR1)*(y('1','1',compR1,Net)-yI('1','1',compR1,Net)))-y('1','1',compR,Net)*sum(comp,Nbonita('1','1',comp,net))   );
Trans2Liq(CompR,net)$(ord(Net)>1 and ord(Net)<card(Net) and ye(net) eq 1)..0=e=(  100*Nbonita('1','1',compR,net)-rhoL('1','1',net)*aI('1','1',net)*sum(compR1,ktransLiq('1','1',net,compR,compR1)*(xI('1','1',compR1,Net)-x('1','1',compR1,Net)))-x('1','1',compR,Net)*sum(comp,Nbonita('1','1',comp,net))   );
Trans3Equil(comp,net)$(ord(Net)>1 and ord(Net)<card(Net) and ye(Net) eq 1)..0=e=((yI('1','1',comp,Net)*P('1','1',Net)*phiI('1','1',comp,Net))-(PsatI('1','1',comp,Net)*gammaI('1','1',comp,Net)*xI('1','1',comp,Net)));
Trans4SumVap(net)$(ord(Net)>1 and ord(Net)<card(Net) and ye(net) eq 1)..0=e=(sum(comp,yI('1','1',comp,Net))-100);
Trans5SumLiq(net)$(ord(Net)>1 and ord(Net)<card(Net) and ye(net) eq 1)..0=e=(sum(comp,xI('1','1',comp,Net))-100);

*RATE-BASED MODEL: Balances de energia
equations EnergyVapor0(net,net1),EnergyLiquid0(net);
EnergyVapor0(net,net1)$((ord(Net)>1 and ord(Net)<card(Net)) and (ord(Net1) eq card(Net1)))..0=e=boilup_flow(net,net1)*HV('1','1',Net1)+V('1','1',Net+1)*HV('1','1',Net+1)-V('1','1',Net)*HV('1','1',Net)-Ebonita('1','1',net);
EnergyLiquid0(net)$(ord(Net)>1 and ord(Net)<card(Net))..0=e=ETin(net)*HFE('1','1',Net)+BUTin(net)*HFB('1','1',Net)+reflux_flow(net)*HL('1','1','1')+L('1','1',Net-1)*HL('1','1',Net-1)-L('1','1',Net)*HL('1','1',Net)+ Ebonita('1','1',net);

*RATE-BASED MODEL: Mecanismo de transferencia de energia
equation ENotrans(net),Etrans1(net),Etrans2(net);
ENotrans(net)$(ord(Net)>1 and ord(Net)<card(Net) and (1-ye(net)) eq 1)..0=e=Ebonita('1','1',net);
Etrans1(net)$(ord(Net)>1 and ord(Net)<card(Net) and ye(net) eq 1)..0=e=( TempV('1','1',Net)-Temp_Dew('1','1',Net)  );
Etrans2(net)$(ord(Net)>1 and ord(Net)<card(Net) and ye(net) eq 1)..0=e=( Temp_Bubble('1','1',Net)-TempL('1','1',Net)  );



equations EquilibrioBubble0(comp,Net),EquilibrioDew0(comp,Net) ,SumaDew0(net),SumaBubble0(net);
SumaBubble0(net)$(ye(net) eq 1)..0=e=(sum(comp,y_Bubble('1','1',comp,Net))-100);
SumaDew0(net)$(ye(net) eq 1)..0=e=(sum(comp,x_Dew('1','1',comp,Net))-100);
...
eqLload(N,j,Net)$(ord(Net)>1 and ord(Net)<card(Net) and yc(net) eq 1)..(1-fracvol)*((3.1415926/4)*(sqr(D)))*Lload(N,j,Net)=e=ul(N,j,net)*Ad ;
...
Weep(N,j,Net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1).. 0=g=(((0.68-0.12)/(sqrt((rhoV(N,j,Net)*sum(comp,MW(comp)*y(N,j,comp,Net)/100)/1000)
...
maxVapVel(N,j,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..unvmax(N,j,Net)=e=(Csbf(N,j,Net)*sqrt(((((sum(comp,rho(N,j,comp,Net)*x(N,j,comp,Net)/100)*sum(comp,MW(comp)*x(N,j,comp,Net)/100)/1000))-(rhoV(N,j,Net)*sum(comp,MW(comp)*y(N,j,comp,Net)/100)/1000)))/(rhoV(N,j,Net)*sum(comp,MW(comp)*y(N,j,comp,Net)/100)/1000)));
positive variable Ffa(N,j,net) "Fraccion de flooding";
equation defFfa(N,j,net);
defFfa(N,j,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..Ffa(N,j,net)=e=(unv(N,j,Net))/(unvmax(N,j,Net));
positive variable alphaBennett(N,j,net) "Densidad relativa de la espuma";
equation defalphaBennett(N,j,net);
defalphaBennett(N,j,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..alphaBennett(N,j,net)=e=exp(-12.55*((unv(N,j,Net)*(sqrt((rhoV(N,j,net)*sum(comp,MW(comp)*(y(N,j,comp,net)/100))*(1/1000))/((rhoL(N,j,net)*sum(comp,MW(comp)*(x(N,j,comp,net)/100))*(1/1000))-(rhoV(N,j,net)*sum(comp,MW(comp)*(y(N,j,comp,net)/100))*(1/1000))))))**0.91));
positive variable hBennett(N,j,net) "Altura de liquido libre de espuma [m]";
equation defhBennett(N,j,net);
defhBennett(N,j,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..hBennett(N,j,net)=e=hD(N,j,Net)+hw;
positive variable Fsa(N,j,net) "Factor superficial [kg^0.5/s*m^0.5]";
equation defFsa(N,j,net);
defFsa(N,j,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..Fsa(N,j,net)=e=unv(N,j,Net)*(sqrt(rhoV(N,j,net)*sum(comp,MW(comp)*(y(N,j,comp,net)/100))*(1/1000)));
positive variable hfroth(N,j,net) "Altura de region de espuma [m]";
equation defhfroth(N,j,net);
defhfroth(N,j,net)$(ord(Net)>1 and ord(Net)<card(Net) and par(net) eq 1)..hfroth(N,j,net)=e=hBennett(N,j,net)/alphaBennett(N,j,net);
...
defkbinvap(N,j,net,comp,comp1)$((ord(Net)>1 and ord(Net)<card(Net)) and (ord(comp) ne ord(comp1)) and par(net) eq 1)..kbinVap(N,j,net,comp,comp1)=e=(((    (0.776+4.57*hw-0.238*(Fsa(N,j,net))+104.8*((L(N,j,Net)*(1/rhoL(N,j,net))*(1/60))/(lw)))/(sqrt((sum(comp3,muV(N,j,comp3,net)*(y(N,j,comp3,net)/100)))/((rhoV(N,j,net)*sum(comp3,MW(comp3)*(y(N,j,comp3,net)/100))*(1/1000))*( (1.013*(10**(-2)))*(TempV.l(N,j,net)**1.75)*((sqrt((MW(comp)+MW(comp1))/(MW(comp)*MW(comp1))))/((P.l(N,j,Net)*(10**5))*(sqr(((Vfuller(comp))**(1/3))+((Vfuller(comp1))**(1/3))))))  ))))      )*((V(N,j,net)*(1/rhoV(N,j,net))*(1/60))/aI(N,j,net))*60));
