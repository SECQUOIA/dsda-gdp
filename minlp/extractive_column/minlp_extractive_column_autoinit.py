...
def minlp_extractive_column(NT=30,  visualize=False):
    ...
    m.Ad = pe.Var(within=pe.NonNegativeReals, initialize=sqrt(pe.value(m.D)/2)*(0.5*(1.854590-0.96)))   # Weir area [m**2]
    m.poro = pe.Var(within=pe.NonNegativeReals, initialize=0.907*pe.sqrt(m.d_hole/pe.value(m.pitch)))   # Plate porosity [*]
