...
def build_small_batch():
    ...
    m.coeff = pe.Param(m.k, initialize=coeff_init, doc='Coefficient for number of parallel units')
    ...
    m.coeffval = pe.Var(m.k, m.j,  within=pe.NonNegativeReals,
                        bounds=(0, pe.log(NK)), doc='Activation of Coefficient')  # Activation of coeff
    m.v = pe.Var(m.j, within=pe.NonNegativeReals, bounds=(
        pe.log(m.vlow), pe.log(m.vupp)), doc='Colume of stage j [L]')  # Volume of stage j [L]
    m.b = pe.Var(m.i, within=pe.NonNegativeReals, doc='Batch size of product i [L]')  # Batch size of product i [L]
    m.tl = pe.Var(m.i, within=pe.NonNegativeReals, doc='Cycle time of product i [hr]')  # Cycle time of product i [hr]
    # Number of units in parallel stage j
    m.n = pe.Var(m.j, within=pe.NonNegativeReals, doc='Number of units in parallel stage j')
